...
void imx6u_clkinit(void)
{
	 ...
	 *              		得出：div_select=    88  
	 */
    CCM_ANALOG->PLL_ARM = (1 << 13) | ((88 << 0) & 0x7f);/* 设置PLL1=1056MHz */
    CCM->CACRR = 1; /* 设置2分频 */
    CCM->CCSR &= ~(1 << 2); /* 设置pll1_sw_clk=pll1_main_clk=1056MHz */
#if 0
	/* 初始化6U的主频为696MHz，也就是NXP宣传的700MHZ主频
	将PLL1设置输出为696MHZ，然后设置为1分频，就把主频设置为696MHZ了
	*/
	/* 设置PLL1=696MHz */ 
    CCM_ANALOG->PLL_ARM = (1 << 13) | ((58 << 0) & 0x7f);
    CCM->CACRR = 0; /* 设置1分频 */
    CCM->CCSR &= ~(1 << 2); /* 设置pll1_sw_clk=pll1_main_clk=696MHz */
#endif
	...
	/* 5、IPG_CLK_ROOT=66MHz,设置IPG_CLK_ROOT最小3Mhz，最大66Mhz (boot rom自动设置好了可以不用设置)*/
	CCM->CBCDR &= ~(3 << 8);	/* CBCDR的IPG_PODF清零 */
	CCM->CBCDR |= 1 << 8;		/* IPG_PODF 2分频，IPG_CLK_ROOT=66MHz */
	
	/* 6、PERCLK_CLK_ROOT=66MHz,设置PERCLK_CLK_ROOT时钟 */
